...
import {useGetPublicPostsQuery} from "../../../store/apis.ts";

export function PostForm() {
    const auth = useAppSelector((state) => state.auth);
    // @ts-ignore
    const postProfileId = auth?.profileId;
    const { data, isLoading: publicLoading } = useGetPublicPostsQuery(postProfileId);
    if (publicLoading === undefined) {
        return <>is Loading</>;
    }
    const posts = data ?? [];
    const normalPosts: Post[] = posts.reduce((acc: Post[], cur: Post) => {
        if (cur.postIsPublished === true && cur.postProfileHandleIsVisible === true) {
            acc.push(cur);
        }
        return acc;
    }, []);
...
//                 />{' '}
//                 <a className="text-white">Public</a>
//             </ul>
//
//             <Card>
//                 <Card.Body>
